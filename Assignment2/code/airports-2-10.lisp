...
(defun UpdateAirport (airport city)
    (cond ((not (includes-airport airport *monitored*)) (format t "Airport ~a does not exist in hash-table ~a.~%" airport (symbol-name `*airports*)))
          ((equal city (gethash airport *airports*)) (format t "Record (~a -> ~a) already exists.~%" airport city))
          (t (format t "Updated (~a -> ~a) to: ~a -> ~a.~%" airport (gethash airport *airports*) airport city) (setf (gethash airport *airports*) city))  
        ))
...
(display-contents *airports* "Values of *airports* Before the Update")
...
(display-contents *airports* "Values of *airports* After the Update")